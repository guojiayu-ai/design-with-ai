  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      const initBubbles = () => {
        ...
        bubbleNodes.forEach(node => {
          ...
          while (!found && attempts < 20) {
            const tempX = Math.random() * (containerWidth - width - 40) + 20;
            const tempY = Math.random() * (containerHeight - height - 40) + 20;
            
            // 检查是否与已占用区域重叠
            const overlaps = occupiedAreas.some(area => {
              const horizontalOverlap = tempX < area.x + area.width + 10 && tempX + width + 10 > area.x;
              const verticalOverlap = tempY < area.y + area.height + 10 && tempY + height + 10 > area.y;
          ...
          node.addEventListener('touchstart', (e) => handleDragStart(e, bubble), { passive: true });
      ...
      const handleDragMove = (e) => {
        // 只在拖动状态下且是鼠标事件时阻止默认行为
        if (dragInfoRef.current.isDragging && e.type === 'mousemove') {
          e.preventDefault();
        }
      ...
      const animateBubbles = () => {
        ...
        bubblesRef.current.forEach(bubble => {
          ...
          // 碰撞检测 - 边界
          // 修改碰撞检测逻辑，添加边界安全距离，防止颤抖
          const bounceBuffer = 5; // 添加缓冲区避免卡在边缘
          
          if (bubble.x <= 20) {
            bubble.x = 20 + bounceBuffer;
            bubble.vx = Math.abs(bubble.vx); // 确保速度为正
            bubble.rotation = Math.random() * 4 - 2; // 减小旋转幅度
          } else if (bubble.x >= containerWidth - bubble.width - 20) {
            bubble.x = containerWidth - bubble.width - 20 - bounceBuffer;
            bubble.vx = -Math.abs(bubble.vx); // 确保速度为负
            bubble.rotation = Math.random() * 4 - 2; // 减小旋转幅度
          }
          
          if (bubble.y <= 20) {
            bubble.y = 20 + bounceBuffer;
            bubble.vy = Math.abs(bubble.vy); // 确保速度为正
            bubble.rotation = Math.random() * 4 - 2; // 减小旋转幅度
          } else if (bubble.y >= containerHeight - bubble.height - 20) {
            bubble.y = containerHeight - bubble.height - 20 - bounceBuffer;
            bubble.vy = -Math.abs(bubble.vy); // 确保速度为负
            bubble.rotation = Math.random() * 4 - 2; // 减小旋转幅度
          }
      ...
      <div className="hero-container">
        <div className="hero-content">
          ...
          <div className="artistic-bubble-container" ref={containerRef}>
            <h3 className="highlight-title bubble-heading" aria-label="课程主题">
              {/* 气泡标题内容 */}
            </h3>
            ...
            <div className="flow-item">Gemini，gpt4o，</div>
            <div className="flow-item">mcp+figma、blender</div>
